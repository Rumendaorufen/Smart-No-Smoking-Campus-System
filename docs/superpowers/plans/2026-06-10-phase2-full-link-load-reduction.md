...
if not exist "%MTX_EXE%" (
    echo [MediaMTX] 错误: 未找到 %MTX_EXE% > con
    echo [MediaMTX] 请将 Windows 版 mediamtx.exe 放置于 web-flask/ 根目录下 > con
    exit /b 1
)
    ...
    def _trigger_alarm_save(self, frame, owner_id, conf):
        ...
        def notify_java():
            try:
                ...
                # 🚀 必须记录失败日志，否则线程静默死亡难以排查
                logger.error(f"Java 中台上报失败 CID={self.camera_id}: {e}")
        # 🚀 使用 daemon 线程防止 Python 退出时阻塞
        # 🚀 Phase 3 多进程架构中，此通知应迁移到 Master 进程的异步任务队列，
        #     通过 multiprocessing.Queue 集中转发，避免每个报警都拉起独立线程
